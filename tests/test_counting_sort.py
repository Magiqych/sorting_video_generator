"""tests/test_counting_sort.py – 計数ソートのテスト。"""
...
class TestCountingSort:
    ...
    def test_basic_sort(self) -> None:
        sorted_vals, _ = counting_sort([3, 1, 2])
        assert sorted_vals == [1, 2, 3]

    def test_events_not_empty(self) -> None:
        _, events = counting_sort([3, 1, 2])
        assert len(events) > 0
